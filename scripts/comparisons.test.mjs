...
test('6.8 mL everyday formats as ≈1.4 teaspoons', () => {
  assert.equal(formatComparison(6.8, 'everyday'), '≈1.4 teaspoons');
});
...
test('1700 mL everyday formats as ≈3.4 water bottles', () => {
  assert.equal(formatComparison(1700, 'everyday'), '≈3.4 water bottles');
});
...
test('8000000 mL household picks jeans and formats as ≈1.1 pairs of jeans', () => {
  ...
  assert.equal(formatComparison(8000000, 'household'), '≈1.1 pairs of jeans');
});
...
test('round-trip: every rung at 1.5× valueMl pluralizes (≈1.5 <plural>)', () => {
  for (const r of LADDER) {
    const set = r.set === 'all' ? 'everyday' : r.set;
    // 1.5× stays under the next rung, so closest-below still picks this rung
    assert.equal(formatComparison(r.valueMl * 1.5, set), `≈1.5 ${PLURALS[r.name]}`, `1.5× ${r.valueMl} mL (${set})`);
  }
});
